...
export default function ChatPanel({ messages, typingUsers, onSend, onTypingStart, onTypingStop }) {
  ...
  return (
    <aside className="glass flex min-h-0 flex-col rounded-lg">
      ...
      <div className="scrollbar-thin min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        {messages.map((message) => (
          <div key={message.id || `${message.username}-${message.timestamp}`} className="rounded-lg bg-white/[0.04] p-3">
            ...
            <p className="break-words text-sm leading-6 text-slate-300">{message.message}</p>
          </div>
        ))}
        <div ref={endRef} />
      </div>

      <form onSubmit={submit} className="relative border-t border-line p-3">
        {showEmojis && (
